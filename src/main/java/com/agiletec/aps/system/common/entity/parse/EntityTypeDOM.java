...
/**
 * This class parses the XML that defines the Entity Types as obtained from the configuration file. This class serves the managers that
 * handle the ApsEntities. This class is the default used in the definition of the Spring bean that defines the base manager {@link
 * ApsEntityManager} of the entities. This declaration of this class must be substituted in the definition of all the services which extend
 * the ApsEntityManager, if a customized DOM class is going to interpret a customized entity. Such entity must have its own implementation
 * in a class that extends ApsEntity. A note about the source code: the naming conflict between the content "Attribute" and the "Attribute"
 * found in the HXML tags makes the distinction between the two difficult. Please pay attention to the correct interpretation of the name of
 * variables and the private methods.
 *
...
public class EntityTypeDOM implements IEntityTypeDOM, BeanFactoryAware {
    ...
    /**
     * Parse the XML element and return the map of Entity Types . This method must be extended to implement particular operations that apply
     * to the specific structure of (an eventually customized) entity class that must be handled by the native Entity manager. That class
     * must implement the IApsEntity interface.
     *
    ...
    protected IApsEntity doParsing(String typeCode, Document document, Class entityClass, IApsEntityDOM entityDom)
    ...
    /**
     * Map the structure of an Entity Type with the attributes defined in its XML configuration item. This method must be extended to
     * implement particular operations that apply to the specific structure of (an eventually customized) entity class that must be handled
    ...
    protected void fillEntityType(IApsEntity entityType, Element currentContentElem) throws ApsSystemException {
        try {
            ...
            for (Element currentAttrElem : attributeElements) {
                ...
                _logger.debug("The Attribute {} of type {} was successfully inserted in the Entity Type {}", attribute.getName(),
                        attribute.getType(), entityType.getTypeCode());
            }
        } catch (Throwable t) {
    ...
    /**
     * Generate an attribute to insert in an Entity Type. The attribute is obtained cloning one of the previously defined elements.
     *
    ...
    private AttributeInterface createAttribute(Element attributeElem) throws ApsSystemException {
        ...
        AttributeInterface attr = getAttributeTypes().get(typeCode);
        if (null == attr) {
