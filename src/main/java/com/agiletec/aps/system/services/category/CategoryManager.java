...
public class CategoryManager extends AbstractService implements ICategoryManager {
    ...
    /**
     * Restituisce una lista piatta delle categorie disponibili, ordinate secondo la gerarchia dell'albero delle categorie. La categoria
     * root non viene inclusa nella lista.
     *
    ...
    public int getMoveTreeStatus() {
        ...
        for (int i = 0; i < utilizers.length; i++) {
            ...
            if (null != moveNodeStatus && moveNodeStatus.containsKey(beanName)
                    && moveNodeStatus.get(beanName) == STATUS_RELOADING_REFERENCES_IN_PROGRESS) {
    ...
    private String[] loadCategoryUtilizers() {
        String[] beans = null;
        try {
            beans = BeanFactoryUtils
                    .beanNamesForTypeIncludingAncestors((ListableBeanFactory) this.getBeanFactory(), CategoryUtilizer.class);
        } catch (Throwable t) {
    ...
    public void startReloadCategoryReferences(String categoryCode) {
        if (this.getMoveTreeStatus() == STATUS_READY) {
            ...
            _logger.warn("Attention: Reload Thread not stated. Expexted status {} but now it's {}", STATUS_READY,
                    STATUS_RELOADING_REFERENCES_IN_PROGRESS);
        }
    }
    ...
    private Thread createAndRunReloadCategoryReferencesThread(String beanName, String categoryCode) {
        ...
        if (status == STATUS_READY) {
            try {
                ...
                String threadName = RELOAD_CATEGORY_REFERENCES_THREAD_NAME_PREFIX + this.getName() + "_" + beanName + "_" + DateConverter
    ...
    protected synchronized void reloadCategoryReferencesByBeanName(String beanName, String categoryCode) throws ApsSystemException {
        ...
        try {
            ...
            if (service != null) {
                ...
                _logger.info("reload category references for {} started at {}", beanName,
                ...
                _logger.info("reload category references for {} end at {}", beanName,
                        DateConverter.getFormattedDate(new Date(), "yyyyMMddHHmmss"));
            }
        } catch (Throwable t) {
    ...
    /**
     * Restituisce la classe DAO specifica per la gestione delle operazioni sulle categorie definite sul db.
     *
    ...
    /**
     * Setta la classe DAO specifica per la gestione delle operazioni sulle categorie definite sul db.
     *
