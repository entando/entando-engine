...
/**
 * Performs the Content Negotiation. Checks whether the request Mime-type is accepted by the user Agent, eventually declaring it. If the
 * Mime-Type is not accepted by the User Agent then the default text/html is declared. The given charset is appended to the declaration
 *
...
public class ContentNegotiationTag extends TagSupport {
  ...
  public int doEndTag() throws JspException {
    ...
    try {
      ...
      String charset = (null != page && null != page.getCharset() && page.getCharset().trim().length() > 0) ? page.getCharset() :
      this.getCharset();
      contentType.append(charset);
    ...
    try {
      ...
      if (null != header) {
        if (null != reqCtx) {
          ...
          String customMimeType = (null != page.getMimeType() && page.getMimeType().trim().length() > 0) ? page.getMimeType() :
          null;
