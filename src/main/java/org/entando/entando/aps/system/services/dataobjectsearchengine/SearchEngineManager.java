...
/**
 * Servizio detentore delle operazioni di indicizzazione di oggetti ricercabili tramite motore di ricerca.
 *
...
public class SearchEngineManager extends AbstractService
    ...
    public void updateFromEntityTypesChanging(EntityTypesChangingEvent event) {
        if (this.getDataObjectManager().getName().equals(event.getEntityManagerName())) {
    ...
    protected boolean verifyReloadingNeeded(IApsEntity oldEntityType, IApsEntity newEntityType) {
        ...
        for (int i = 0; i < attributes.size(); i++) {
            ...
            boolean isOldAttributeIndexeable = (oldAttribute.getIndexingType() != null && oldAttribute.getIndexingType()
                    .equalsIgnoreCase(IndexableAttributeInterface.INDEXING_TYPE_TEXT));
            boolean isNewAttributeIndexeable = (newAttribute != null && newAttribute.getIndexingType() != null && newAttribute
                    .getIndexingType().equalsIgnoreCase(IndexableAttributeInterface.INDEXING_TYPE_TEXT));
    ...
    //@Override
    public List<String> searchId(String sectionCode, SearchEngineFilter[] filters, Collection<String> allowedGroups)
            throws ApsSystemException {
        return this.searchEntityId(filters, null, allowedGroups);
    }

    //@Override
    public List<String> searchEntityId(SearchEngineFilter[] filters, Collection<ITreeNode> categories, Collection<String> allowedGroups)
            throws ApsSystemException {
    ...
    //@Override
    public FacetedContentsResult searchFacetedEntities(SearchEngineFilter[] filters, Collection<ITreeNode> categories,
            Collection<String> allowedGroups) throws ApsSystemException {
