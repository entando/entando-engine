...
public class ApiDataObjectModelInterface extends AbstractApiDataObjectInterface {
    ...
    public StringListApiResponse getModels(Properties properties) throws Throwable {
        ...
        try {
            ...
            if (null != dataType && null == this.getDataObjectManager().getSmallDataTypesMap().get(dataType)) {
                ApiError error = new ApiError(IApiErrorCodes.API_PARAMETER_VALIDATION_ERROR, "Content Type " + dataType + " does not exist",
    ...
    public DataObjectModel getModel(Properties properties) throws Throwable {
        ...
        try {
            ...
            throw new ApiException(IApiErrorCodes.API_PARAMETER_VALIDATION_ERROR,
                    "Invalid number format for 'id' parameter - '" + idString + "'", Response.Status.CONFLICT);
        }
        DataObjectModel model = this.getDataObjectModelManager().getDataObjectModel(id);
        if (null == model) {
            throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "Model with id '" + idString + "' does not exist",
                    Response.Status.CONFLICT);
        }
        return model;
    }

    public void addModel(DataObjectModel model) throws Throwable {
        if (null != this.getDataObjectModelManager().getDataObjectModel(model.getId())) {
            throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "Model with id " + model.getId() + " already exists",
                    Response.Status.CONFLICT);
        }
        if (null == this.getDataObjectManager().getSmallDataTypesMap().get(model.getDataType())) {
            throw new ApiException(IApiErrorCodes.API_PARAMETER_VALIDATION_ERROR, "Content Type " + model.getDataType() + " does not exist",
                    Response.Status.CONFLICT);
        }
        try {
    ...
    public void updateModel(DataObjectModel model) throws Throwable {
        DataObjectModel oldModel = this.getDataObjectModelManager().getDataObjectModel(model.getId());
        if (null == oldModel) {
            throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "Model with id " + model.getId() + " does not exist",
                    Response.Status.CONFLICT);
        }
    ...
    public void deleteModel(Properties properties) throws Throwable {
        String idString = properties.getProperty("id");
        int id = 0;
        try {
            id = Integer.parseInt(idString);
