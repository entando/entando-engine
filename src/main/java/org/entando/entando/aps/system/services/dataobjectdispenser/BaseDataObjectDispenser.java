...
/**
 * Fornisce i DataObject formattati. Il compito del servizio, in fase di richiesta di un DataObject formattato, è quello di controllare
 * preliminarmente le autorizzazzioni dell'utente corrente all'accesso al DataObject; successivamente (in caso di autorizzazioni valide)
 * restituisce il DataObject formattato.
 *
...
public class BaseDataObjectDispenser extends AbstractService implements IDataObjectDispenser {
    ...
    //    key = "T(com.agiletec.plugins.jacms.aps.system.services.dispenser.BaseContentDispenser).getRenderizationInfoCacheKey
    //    (#contentId, #modelId, #langCode, #reqCtx)")
    //@CacheableInfo(groups = "T(com.agiletec.plugins.jacms.aps.system.services.dispenser.BaseContentDispenser)
    // .getRenderizationInfoCacheGroupsCsv(#contentId, #modelId)")
    ...
    protected DataObjectRenderizationInfo getRenderizationInfo(PublicDataTypeAuthorizationInfo authInfo,
        ...
        try {
            UserDetails currentUser = (null != reqCtx) ? (UserDetails) reqCtx.getRequest().getSession()
                    .getAttribute(SystemConstants.SESSIONPARAM_CURRENT_USER) : null;
            List<Group> userGroups =
                    (null != currentUser) ? this.getAuthorizationManager().getUserGroups(currentUser) : new ArrayList<Group>();
    ...
    public static String getRenderizationInfoCacheKey(String dataObjectId, long modelId, String langCode, RequestContext reqCtx) {
        UserDetails currentUser = (null != reqCtx) ? (UserDetails) reqCtx.getRequest().getSession().getAttribute(SystemConstants
        .SESSIONPARAM_CURRENT_USER) : null;
