...
public class ApiPageModelInterface implements BeanFactoryAware, IApiExportable {
        ...
        try {
            ...
            if (null != pageModels) {
                ...
                while (iter.hasNext()) {
                    ...
                    String url = this
                            .getApiResourceUrl(pageModel, properties.getProperty(SystemConstants.API_APPLICATION_BASE_URL_PARAMETER),
                                    properties.getProperty(SystemConstants.API_LANG_CODE_PARAMETER),
                                    (MediaType) properties.get(SystemConstants.API_PRODUCES_MEDIA_TYPE_PARAMETER));
    ...
    public PageModel getPageModel(Properties properties) throws Throwable {
        String code = properties.getProperty("code");
        PageModel pageModel = null;
        try {
            pageModel = this.getPageModelManager().getPageModel(code);
            if (null == pageModel) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "PageModel with code '" + code + "' does not exist",
    ...
    public void addPageModel(PageModel pageModel) throws Throwable {
        try {
            if (null != this.getPageModelManager().getPageModel(pageModel.getCode())) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR,
                        "PageModel with code " + pageModel.getCode() + " already exists", Response.Status.CONFLICT);
            }
    ...
    public void updatePageModel(PageModel pageModel) throws Throwable {
        try {
            if (null != this.getPageModelManager().getPageModel(pageModel.getCode())) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR,
                        "PageModel with code '" + pageModel.getCode() + "' does not exist", Response.Status.CONFLICT);
            }
    ...
    public void deletePageModel(Properties properties) throws Throwable {
        String code = properties.getProperty("code");
        try {
            ...
            if (!references.isEmpty()) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR,
                        "PageModel with code " + code + " has references with other object", Response.Status.CONFLICT);
            }
