...
public class ApiGuiFragmentInterface implements BeanFactoryAware, IApiExportable {
    ...
    public List<LinkedListItem> getGuiFragments(Properties properties) throws Throwable {
        ...
        try {
            ...
            for (int i = 0; i < codes.size(); i++) {
                ...
                String url = this
                        .getApiResourceUrl(fragmantCode, properties.getProperty(SystemConstants.API_APPLICATION_BASE_URL_PARAMETER),
    ...
    public JAXBGuiFragment getGuiFragment(Properties properties) throws Throwable {
        ...
        try {
            ...
            if (null == guiFragment) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "GuiFragment with id '" + code + "' does not exist",
        ...
        try {
            if (null != this.getGuiFragmentManager().getGuiFragment(jaxbGuiFragment.getCode())) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR,
                        "GuiFragment with code " + jaxbGuiFragment.getCode() + " already exists", Response.Status.CONFLICT);
            }
    ...
    public void updateGuiFragment(JAXBGuiFragment jaxbGuiFragment) throws Throwable {
        try {
            if (null == this.getGuiFragmentManager().getGuiFragment(jaxbGuiFragment.getCode())) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR,
                        "GuiFragment with code '" + jaxbGuiFragment.getCode() + "' does not exist", Response.Status.CONFLICT);
            }
            GuiFragment guiFragment = jaxbGuiFragment.getGuiFragment();
            if (StringUtils.isBlank(guiFragment.getCurrentGui())) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "one between A and B must be valued", Response.Status.CONFLICT);
    ...
    public void deleteGuiFragment(Properties properties) throws Throwable {
        ...
        try {
            GuiFragment guiFragment = this.getGuiFragmentManager().getGuiFragment(code);
            if (null == guiFragment) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "GuiFragment with code '" + code + "' does not exist",
                        Response.Status.CONFLICT);
            }
            if (guiFragment.isLocked()) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR, "GuiFragment with code '" + code + "' are locked",
                        Response.Status.CONFLICT);
            }
            ...
            if (!references.isEmpty()) {
                throw new ApiException(IApiErrorCodes.API_VALIDATION_ERROR,
                        "GuiFragment with code " + code + " has references with other object", Response.Status.CONFLICT);
            }
