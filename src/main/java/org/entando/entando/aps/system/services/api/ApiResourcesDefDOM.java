    ...
    private void checkResource(ApiResource resource, Map<String, ApiResource> apiResources) {
        try {
            ...
            if (null != extractedResource) {
                _logger.error(
                        "ALERT: There is more than one API with namespace '{}', resource '{}' into the same definitions file - The second"
                                + " definition will be ignored!!!",
                        resource.getNamespace(), resource.getResourceName());
            } else {
    ...
    private void checkMethod(ApiMethod apiMethod, ApiResource resource) {
        try {
            ...
            if (null != extractedMethod) {
                _logger.error(
                        "ALERT: There is more than one API method {} for resource '{}' into the same definitions file - The second "
                                + "definition will be ignored!!!",
                        apiMethod.getHttpMethod(), apiMethod.getResourceName());
            } else {
                resource.setMethod(apiMethod);
            }
        } catch (Exception e) {
