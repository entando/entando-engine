...
class FileBrowserControllerIntegrationTest extends AbstractControllerIntegrationTest {
    ...
    void testBrowseFolder_1() throws Exception {
        ...
        result.andExpect(jsonPath("$.payload[0].name", is("Entando")));
        result.andExpect(jsonPath("$.payload[0].protectedFolder", is(false)));
        result.andExpect(jsonPath("$.payload[0].directory", is(true)));
        result.andExpect(jsonPath("$.payload[1].name", is("conf")));
        result.andExpect(jsonPath("$.payload[1].protectedFolder", is(false)));
        result.andExpect(jsonPath("$.payload[1].directory", is(true)));
        result.andExpect(jsonPath("$.payload[2].name", is("entando_logo.jpg")));
        result.andExpect(jsonPath("$.payload[2].protectedFolder", is(false)));
        result.andExpect(jsonPath("$.payload[2].directory", is(false)));
        result.andExpect(jsonPath("$.errors", Matchers.hasSize(0)));
